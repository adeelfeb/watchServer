...
import { parseAndStoreInPinecone, getVectorFromPinecone, getVectorFromPineconeByFileId } from "./pineConeVectorSaving.controller.js";
...
const addFileData = asyncHandler(async (req, res) => {
  try {
    ...
    new Promise(async (resolve, reject) => {
      try {
        const pineconeVectorId = await parseAndStoreInPinecone(fileData._id, fileUrl, userId);
...
const getVectorData = asyncHandler(async (req, res) => {
  try {
    ...
    if (!query || typeof query !== "string") {
      return res.status(400).json({
        status: 400,
        message: "Invalid or missing 'query' in request body",
      });
    }
    ...
    if (!vectorData || vectorData.length === 0) {
      return res.status(404).json({
        status: 404,
        message: `No vector data found${fileId ? ` for file ID: ${fileId}` : ""}.`,
      });
    }
